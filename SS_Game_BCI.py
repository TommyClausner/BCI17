...
# Images
alien_red = pygame.image.load("alien_red.png").convert_alpha()
alien_red = pygame.transform.scale(alien_red, (int(ScreenWidth * 0.15), int(ScreenHeight * 0.15)))
alien_blue = pygame.image.load("alien_blue.png").convert_alpha()
...
alien_bonus = pygame.image.load("alien_green.png").convert_alpha()
alien_bonus = pygame.transform.scale(alien_bonus, (int(ScreenWidth * 0.1), int(ScreenHeight * 0.1)))


space_ship = pygame.image.load("space_ship.png").convert_alpha()
                    ...
                    for u in list(reversed(range(len(balls)))):
                        if len(bonusaliens) > 0 and len(balls) > 0:
                            if hit_alien(bonusaliens[i], balls[u]):
                                explosion_pos = (bonusaliens[i]._x - bonusaliens[i].size*2, bonusaliens[i]._y_now - bonusaliens[i].size*2)
                                show_explosion = 1
                                del bonusaliens[i]
                                del balls[u]
                                hit_score += 1
            ...
            for i in list(reversed(range(len(balls)))):
                if len(aliens) > 0:
                    if hit_alien(aliens[0], balls[i]):
                        explosion_pos = (aliens[0]._x - aliens[0].size, aliens[0]._y_now - aliens[0].size)
                        show_explosion = 1
                        del aliens[0]
                        del balls[i]
                        hit_score += 1
